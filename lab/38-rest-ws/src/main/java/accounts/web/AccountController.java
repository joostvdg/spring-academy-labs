...
public class AccountController {
	 ...
	 */
	private ResponseEntity<Account> entityWithLocation(Long accountId) {
	...
	public ResponseEntity<Void> addBeneficiary(@PathVariable long accountId, @RequestBody String beneficiaryName) {
		...
		String cleanedName = beneficiaryName.toLowerCase(Locale.ROOT).trim();
		accountManager.addBeneficiary(accountId, cleanedName);
		URI location = ServletUriComponentsBuilder
				.fromCurrentRequest()
				.path("/{beneficiaryName}")
				.buildAndExpand(cleanedName)
				.toUri();
		return ResponseEntity.created(location).build();
	}
	...
	public void handleDataIntegrityViolation(Exception e) {
