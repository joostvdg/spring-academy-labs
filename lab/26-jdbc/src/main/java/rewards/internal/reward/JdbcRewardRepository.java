...
//		}
		...
		var rewardAmount  = contribution.getAmount().asBigDecimal();
		var rewardDate = new Date(SimpleDate.today().inMilliseconds());
		var diningDate = new Date(dining.getDate().inMilliseconds());
		var diningAmount = dining.getAmount().asBigDecimal();

		var updateResult = jdbcTemplate.update(sql,
				confirmationNumber,
				rewardAmount,
				rewardDate,
		...
		if (updateResult != 1) {
			...
			System.out.println("Failed to insert reward record");
		}
...
//		}
		return jdbcTemplate.queryForObject(sql, String.class).toString();
	}
}
